...
void ofApp::update() {
	...
	vector<ofColor> color_list = { ofColor(217, 197, 137),   ofColor(128, 169, 53), ofColor(91, 20, 52) };
	...
	for (int i = 0; i < 3; i++) {
		...
		float threshold_1 = ofMap(i, 0, 3, 0.35, 0.48);
		float threshold_2 = ofMap(i, 0, 3, 0.55, 0.52);

		for (float phi_deg = 0; phi_deg < 360; phi_deg += phi_deg_step) {

			for (float theta_deg = 0; theta_deg < 360; theta_deg += theta_deg_step) {

				auto noise_value = ofNoise(glm::vec4(this->make_point(R, r, theta_deg, phi_deg) * 0.0085, noise_seed + ofGetFrameNum() * 0.025));
				if (noise_value <= threshold_1 || noise_value >= threshold_2) { continue; }

				auto noise_1 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg - theta_deg_step, phi_deg) * 0.0085, noise_seed + ofGetFrameNum() * 0.025));
				auto noise_2 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg, phi_deg + phi_deg_step) * 0.0085, noise_seed + ofGetFrameNum() * 0.025));
				auto noise_3 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg, phi_deg - phi_deg_step) * 0.0085, noise_seed + ofGetFrameNum() * 0.025));
				auto noise_4 = ofNoise(glm::vec4(this->make_point(R, r, theta_deg + theta_deg_step, phi_deg) * 0.0085, noise_seed + ofGetFrameNum() * 0.025));
				...
				for (int k = 0; k < 4; k++) {
					...
					this->face.addColor(color_list[i]);
				}
